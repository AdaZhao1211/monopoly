...
class ClientSM:
    ...
    def connect_to(self, peer):
        ...
        if response == (M_CONNECT+'ok'):
            ...
            self.out_msg += 'If you wish to start the game, simply type * game * (no asterix)\n'
...
#==============================================================================
            ...
            if len(my_msg) > 0:     # My stuff, going out
                if my_msg == "ROLL":
                    mysend(self.s, M_ROLL)
                elif my_msg == "QUIT":
                    mysend(self.s, M_QUITGAME)
                elif my_msg == "STATUS":
                    mysend(self.s, M_STATUS)
                else:
                    mysend(self.s, M_EXCHANGE + "[" + self.me + "] " + my_msg)
            if peer_code == M_TOCHAT:
                self.state = S_CHATTING
            elif peer_code == M_TOBUY:
                self.state = S_BUYING
            if len(peer_msg) > 0:   # Peer's stuff, coming in
                self.out_msg += peer_msg
        elif self.state == S_BUYING:
            if len(my_msg) > 0:
                if my_msg == "BUY":
                    mysend(self.s, M_BUY)
                elif my_msg == "NO":
                    mysend(self.s, M_NOTBUY)
                elif my_msg == "STATUS":
                    mysend(self.s, M_STATUS)
            if peer_code == M_TOGAME:
                self.state = S_PLAYING
